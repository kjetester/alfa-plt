...
public class InvolvementsTest extends InvolvementsBaseTest {
  ...
  public void involvementsDisabledExperimentTest(
    ...
    createOption(false, null, experiment.getUuid(), .5D, getContentManager());
    // TEST //
  ...
  public void involvementsCancelledExperimentTest(
    ...
    final var experiment = createExperiment(
        experimentDevice, pageId, getRandomProductType(), experimentEnd, .5D, getContentManager());
    final var option1 = createOption(true, List.of(widget1.getUid()), experiment.getUuid(), .5D, getContentManager());
    final var option2 = createOption(false, null, experiment.getUuid(), .5D, getContentManager());
    final var runningExperiment = runExperimentAssumingSuccess(experiment, getContentManager());
    stopExperimentAssumingSuccess(runningExperiment, getContentManager());
    // TEST //
    Response response = getInvolvements(pageId, clientDevice, geos, getContentManager());
    assertThat(response.getBody().asString())
        .as("Проверка отсутствия переданных данных какого-либо эксперимента")
