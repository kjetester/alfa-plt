...
import ru.alfabank.platform.buisenessobjects.ContentPageControllerResponseResponse;
...
public class BasePageTest extends BaseTest {
  ...
  protected void comparePagesInContentPageController(
    ...
    LOGGER.info(String.format(
        ...
        sourcePage.getUri(),
        createdPage.getUri()));
    List<ContentPageControllerResponseResponse.Widget> expected =
        getPageFromContentPageController();
    List<ContentPageControllerResponseResponse.Widget> actual =
   ...
   */
  protected List<ContentPageControllerResponseResponse.Widget> getPageFromContentPageController(
      final Page... page) {
    ...
    LOGGER.info(String.format(
        ...
        response.prettyPrint()));
    assertThat(response.statusCode()).isEqualTo(200);
    ContentPageControllerResponseResponse contentPageControllerResponse = response
        .as(ContentPageControllerResponseResponse.class);
  ...
  protected void checkCreatedPageAtPageController(Page page) {
    ...
    if (page.getTeamList() == null) {
      ...
      softly
          .assertThat(pageControllerJson.getList("teams"))
          .containsAll(page.getTeamList().stream().map(Team::getId).collect(Collectors.toList()));
    }
    softly.assertAll();
  }
